...
def get_df(file):
    ...
    for root, dirs, files in os.walk(file):
        for file in files:
            ...
            title = doc.paragraphs[0].text
            i+=1
            print('第'+ str(i) +'个文章'+title)

            head.append(title)

            txt = ''
            for j in range(len(doc.paragraphs)-5):
                print("第" + str(j) + "段的内容是：" + doc.paragraphs[j].text)

                txt += doc.paragraphs[j].text
            #txt.replace(u'\u3000', u'').replace('\n', '').replace('\r', '').replace(" ", '')

            #txt.replace(u'\u3000', u'').replace('\n', '').replace('\r', '').replace(" ", '')
            print('正文：---------------------------------------------------------------------------------------------------')
            txt1 = txt.replace(u'\u3000', u'').replace('\n', '').replace('\r', '').replace(" ", '')
            content.append(txt1)
            print(txt1)
        df = pd.DataFrame({'title': head, "content": content})
    return df
if __name__ == '__main__':

    # path = r'D:\PycharmProjects\other\doc2docx\上海\上海市交通委员会关于征集2020年度本市交通运输行业重点节能低碳技术的通知(FBMCLI.14.1658519).docx'
    #
    # doc = getText(path)
    # print(doc)
# --------------------------------------------------------------------
    ...
    #         print(txt1)
    # df = pd.DataFrame({'title': head, "content": content})
    #df.to_csv(r'D:\PycharmProjects\other\test2\shanghai.csv')
    #print(content)
#单独测试有问题省份
    # head=[]
    # context=[]
...
#真
    ...
    for root, dirs, files in os.walk(path2):
        for dir in dirs:
            ...
            print('这个是file：'+ file)
            df = get_df(file)
            df.to_csv(r'D:\PycharmProjects\other\docx_treated\{}.csv'.format(dir), index=True)
            print(df)
            # df = get_province(file)
            # df.to_csv(r'D:\PycharmProjects\other\docx_treated')
            # locals()['df_{}'.format(dir)]= get_province(file)
            # locals()['df_{}'.format(dir)].csv(r'D:\PycharmProjects\other\test_data\{}.csv'.format(dir), index=False)
